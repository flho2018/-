        ...
        try {
          ...
          setter(prev => {
            ...
            rows.forEach((r, i) => {
              ...
              // =============================================================
              //  دليلٌ قاطع يسبق التخمين الزمني: هل هذا السجل بانتظار الرفع؟
              // =============================================================
              //  الختم الزمني **تخمين**: «قديم ⇒ غالباً حُذف من جهاز آخر».
              //  وهو مطابق لنمط العطل الأصلي (المحذوف قديم دائماً)، لكنه يخطئ
              //  في حالة واحدة خطيرة: جهاز كتب فاتورة ثم انقطع اتصاله، فلمّا
              //  عاد وصلت اللقطة قبل أن تُفرَّغ طابور الكتابة — فالفاتورة
              //  قديمة الختم وغائبة عن اللقطة، فتُسقَط وهي لم تُحذف قطّ.
              //  و`pendingLocal` يحمل ما كتبناه ولم تؤكّده السحابة بعد، فهو
              //  دليل مباشر لا تخمين. ما دام فيه: لا يُمسّ مهما بدا قديماً.
              //  ⚠️ ذاكرته تزول بإعادة التحميل، فلا يغطّي ما كُتب قبل إقلاع
              //  هذه الجلسة — هناك يبقى طابور Firestore نفسه هو الضامن.
              // =============================================================
              const pendingForKey = syncEngine.pendingLocal?.[key];
              const isAwaitingUpload = Boolean(pendingForKey && pendingForKey.has(rid));

              if (!isAwaitingUpload && cloudSeenAt && stampOfRow(r) < cloudSeenAt - CLOUD_GRACE_MS) {
                dropped++;
