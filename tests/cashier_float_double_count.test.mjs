...
// ---------------------------------------------------------------------
//  العهدة المُسلّمة من المدير: مالُ صاحبها، وحدٌّ أدنى لا يدهس ما بذمّته
// ---------------------------------------------------------------------
console.log('\n— العهدة المُسلّمة تُنسب لصاحبها ولا تمحو نقده —');
{
  const ctx = fs.readFileSync('src/context/AppContext.jsx', 'utf8');

  // كانت `myFloats.length > 0 ? myFloatTotal : startCash` تمحو ما بذمّة
  // الكاشير: يحمل ٢٧٥ ويسلّمه المدير ١٠٠ فكّة ⇒ يفتح بـ١٠٠ وتتبخّر ٢٧٥
  t('العهدة صارت حدّاً أدنى لا قيمة تدهس الرصيد',
    /Math\.max\(Number\(startCash\)\s*\|\|\s*0,\s*myFloatTotal\)/.test(ctx) ? 1 : 0, 1);
  t('لم تعد العهدة تُلغي الرصيد المُدخل',
    /myFloats\.length > 0 \? myFloatTotal : \(Number\(startCash\)/.test(ctx) ? 0 : 1, 1);

  // وبدون طرح العهدة كان الترحيل يُسقط معلّقاً لم يُرحَّل فعلاً
  t('المُرحَّل يُحسب بعد طرح العهدة',
    /carriedFromPrev\s*=\s*roundMoney\(Math\.max\(0,\s*effectiveStartCash\s*-\s*myFloatTotal\)\)/.test(ctx) ? 1 : 0, 1);
  t('التسوية تعمل على الجزء المُرحَّل لا على كامل الرصيد',
    /if \(carriedFromPrev > 0\.005\)/.test(ctx) ? 1 : 0, 1);

  // العهدة المعلّقة كانت تظهر في صندوق داخل بطاقة المدير بلا صاحب
  t('العهدة المعلّقة تدخل رصيد صاحبها',
    /subType === 'drawer_funding' && t\.status === 'pending'[\s\S]{0,300}?'floatCash'/.test(ctx) ? 1 : 0, 1);
  t('مجموع رصيد الكاشير يشمل العهدة',
    /row\.openCash \+ row\.pendingCash \+ row\.floatCash/.test(ctx) ? 1 : 0, 1);
}
...
}
{
  ...
  // =====================================================================
  //  دمج IndexedDB: لا يُسقط سجلاً بانتظار الرفع
  // =====================================================================
  //  الإسقاط يعتمد ختماً زمنياً — وهو تخمين يخطئ على جهاز كتب فاتورة ثم
  //  انقطع اتصاله: الفاتورة قديمة الختم وغائبة عن اللقطة فتُسقَط وهي لم
  //  تُحذف. `pendingLocal` دليل مباشر يسبق التخمين.
  t('الدمج يفحص pendingLocal قبل الإسقاط',
    /pendingLocal\?\.\[key\]/.test(ctx) ? 1 : 0, 1);
  t('السجل المنتظر رفعه يُستثنى من شرط الإسقاط',
    /!isAwaitingUpload\s*&&\s*cloudSeenAt/.test(ctx) ? 1 : 0, 1);
  t('لا إسقاط إطلاقاً قبل وصول أول لقطة سحابية',
    /if \(!isAwaitingUpload && cloudSeenAt &&/.test(ctx) ? 1 : 0, 1);
}
