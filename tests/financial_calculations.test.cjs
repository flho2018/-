// ⛔ لم يعد هذا الملف ضمن `npm test` — استُبدل بـ `tests/financial_real.test.mjs`
//
//  السبب: هذا الملف **ينسخ** الدوال المالية بدل أن يستوردها. فكانت اختباراته
//  السبعة عشر تمرّ خضراء على نسخةٍ من المنطق قد تكون متخلّفة عن الملف الحقيقي
//  بأشهر — اختبارٌ يطمئن ولا يحمي، وهو أسوأ من غياب الاختبار لأن غيابه معروف.
//
//  والدليل العملي: البديل الجديد — الذي يُشغّل `getCartTotals` الحقيقية —
//  كشف في أول تشغيل أن سعراً غير رقمي يجعل **إجمالي الفاتورة كلها NaN**.
//  عطلٌ عاش هنا دون أن تلمسه سبعة عشر اختباراً «ناجحاً».
//
//  يُحتفظ به للرجوع إلى حالات الاختبار نفسها (ديون FIFO مثلاً لم تُنقل بعد).
